...
class ProjectDatabase {
     ...
     * @private
     */
    private _validateProject(project: any): { valid: true } | { valid: false, message: string } {
        try {
    ...
    async get(id: number, version?: number): Promise<{ data: SimplifiedProject, version: number }> {
        ...
        const data: SimplifiedProject = JSON.parse(json);

        // add image file static urls
        const imageFiles = await this.listImages(id, v);
    ...
    async add(id: number, version: number, data: SimplifiedProject): Promise<void> {
        if (!this._validateProject(data)) throw new Error(`Project structure not valid`);
